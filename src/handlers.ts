...
export function graphqlHandler(event: APIGatewayProxyEvent, lambdaContext: Context, callback: Callback): void | Promise<APIGatewayProxyResult> {
    lambdaContext.callbackWaitsForEmptyEventLoop = false;

    function callbackFilter( error: CallbackError, output: CallbackOutput ): void {
        if ( ! output || error ) {
            return;
        }
